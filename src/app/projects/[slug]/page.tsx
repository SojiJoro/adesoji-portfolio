...
const projectData: Record<string, Project> = {
  'rds-failover': {
    ...
    details: [
      ...
      },
      {
        ...
        content:
          'We deployed an Aurora Global Database with a secondary writer in eu-central-1. A Route 53 health check watches the primary writer endpoint. If it fails, DNS automatically flips to the secondary’s endpoint within 60 seconds.'
      },
      {
        heading: 'Implementation Steps',
        content:
          '1. Wrote Terraform modules to provision Global Database resources and DNS failover. 2. Configured CloudWatch alarms on replica lag > 5 seconds. 3. Automated IAM roles for cross‐region KMS key access. 4. Deployed via CI/CD pipeline using GitHub Actions.'
      },
      {
        heading: 'Testing & Validation',
        content:
          'Performed simulated failover drills by forcing a primary node reboot. Logged RTO (Recovery Time Objective) and verified zero data loss. Documented the playbook and trained on-call engineers.'
      },
      {
      ...
      },
      {
        ...
        content:
          'Terraform state locking across multiple regions required careful S3 bucket versioning and DynamoDB lock tables. We standardised naming conventions to avoid drift.'
      },
      {
        heading: 'Next Steps',
        content:
          'Integrate Lambda for automated failback once the primary returns healthy. Add secondary failover region in ap-southeast-2 to cover Asia Pacific.'
      },
      {
  ...
  'airtable-asset': {
    ...
    description:
      'Designed an Airtable-based hardware tracker integrated with Confluence, reducing asset loss by 40%.',
    details: [
      {
        heading: 'Overview',
        content:
    ...
    <section className="section bg-white">
      <div className="container max-w-3xl space-y-8">
        ...
        {project.details.map(({ heading, content }) => (
          <div key={heading} className="mt-large">
